function [Me] = hHEX_8M(ex, ey, ez, rho, integrationRule)
ex = reshape(ex, [], 1);
ey = reshape(ey, [], 1);
ez = reshape(ez, [], 1);
...
N = ones(nbrGaussPnts, 8);
% Assign to all gaussian points at the same time (thereof 1:3:end)
N(:, [1 2 5 6]) = N(:, [1 2 5 6]) .* (1-eta);
N(:, [3 4 7 8]) = N(:, [3 4 7 8]) .* (1+eta);
N(:, [1 4 5 8]) = N(:, [1 4 5 8]) .* (1-xi);
N(:, [2 3 6 7]) = N(:, [2 3 6 7]) .* (1+xi);
N(:, 1:4) = N(:, 1:4) .* (1-zeta);
N(:, 5:8) = N(:, 5:8) .* (1+zeta);
N(:) = N / 8;
...
for i = 1:nbrGaussPnts
    ...
    Me(:) = Me + N(i, :)' * rho * N(i, :) * detJ * weights(i);
end
end