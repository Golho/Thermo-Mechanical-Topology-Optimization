...
% generering av b, villkorsvektor
...
b=[b Wlast Wu*ones(1,8) Wl*ones(1,8) WT];
b(73)=b(73)-W0;
b=b';
...
sense=[ones(1,24) 2*ones(1,24) 2*ones(1,24) zeros(1,8) ones(1,8) 2*ones(1,8) 2]'; 
...
A=[A;A2 A2 A2 A1 A1 A1 diag(alfa*ones(1,7),-1)-A1];
A=[A;zeros(8,48) A1];
A=[A;zeros(8,48) A1];
A=[A;zeros(1,55) 1];
A=sparse(A);
...
for i=1:3
qp=qp + [x(i*8+17:i*8+24) x(i*8-7:i*8)];
%q=q + [x(i*8+17:i*8+24)];
end

franlager=alfa*[W0; x(49:55)]-x(49:56);
plotvec=[qp(:,1)+franlager qp  et' x(49:56) Wlast']
...
forandringkostnad=lamda'*deltab
